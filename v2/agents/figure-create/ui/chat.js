...
window.Chat = (function () {
  ...
  function hero() {
    ...
    d.id = 'hero'
    d.innerHTML =
      '<h1>Describe a figure</h1>' +
      '<p>Tell Figure Create what to draw — a mechanism, a pathway, an anatomy plate, a process. Renders publication-grade artwork with editable labels. Click any generated figure to edit it on a canvas.</p>' +
  ...
  /** Attach a data URL as a file to the next message. Used by the canvas editor
   *  to send the edited figure back to the agent. */
  function attachDataUrl(dataUrl, name) {
    const mime = dataUrl.split(';')[0].split(':')[1] || 'image/png'
    const base64 = dataUrl.split(',')[1] || ''
    pending.push({ name, mimeType: mime, dataBase64: base64 })
    drawAttachments()
  }

  return { init, reset, open, ask, attachDataUrl, get sessionKey() { return sessionKey } }
})()
