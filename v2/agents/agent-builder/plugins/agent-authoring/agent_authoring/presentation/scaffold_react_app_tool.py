...
class ScaffoldReactAppTool(Tool):
    ...
    description = (
        "START A REACT APP for an agent: writes agents/<id>/app/ with package.json, "
        "vite.config.ts, tsconfig.json, index.html and the SDK vendored inside it. It writes NO "
        "components and NO src/ — you write those, after reading the sample agents.\n"
        "USE THIS over scaffold_ui when the window needs more than a conversation: several "
        "views, an artifact beside the chat, panels driven by direct tool calls. Use scaffold_ui "
        "when a plain chat window is genuinely the whole product — it copies a complete app and "
        "needs no build step or Node.\n"
