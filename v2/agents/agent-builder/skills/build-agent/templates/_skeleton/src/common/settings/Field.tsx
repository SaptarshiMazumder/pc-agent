...
export function Field({
  ...
  spec: FieldSpec
  value: unknown
  /** Which layer this value came from, for an agent-scoped field. THREE now: the person
   *  running the agent, the person who wrote it, or the machine. */
  source?: 'this agent' | 'author' | 'daemon'
  ...
  pinnedBy?: string
  /** The agent's author set this and did not make it editable.
   *
   *  IT HAS TO DISABLE THE CONTROL, not just label it. The daemon refuses the write either way —
   *  `config.set` drops a locked key before it reaches any of the three write paths — so a row
   *  left editable is a row you can change, save, and watch silently revert. A control that does
   *  nothing is worse than one that is plainly not yours to touch. */
  lockedByAuthor?: boolean
  catalog?: Array<string | CatalogOption>
  onChange: (value: unknown) => void
}) {
  const options = spec.catalog ? catalog : spec.options
  const disabled = !!pinnedBy || !!lockedByAuthor
    ...
    <div className="field">
      <div>
        ...
        {pinnedBy ? (
          ...
          /* WHY it cannot be changed, in the words of the thing that decided. "Set by this
             agent's author" is a fact somebody can act on — they can ask the author, or use a
             different agent. A greyed control with no reason reads as a bug in the page. */
          <span className="fhelp pinned">set by this agent&rsquo;s author</span>
        ) : (
          source && (
