...
Every agent with a window gets this whole folder as `app/src/common/` — copied in when the
agent is CREATED, along with the rest of its window. THE `.css` FILES ARE YOURS — restyle any of these screens freely; only the `.tsx` is
compared against the source. Import from
