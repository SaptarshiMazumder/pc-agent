...
;(function () {
  ...
  // ── suggestions ───────────────────────────────────────────────────────────
  // One opener per CAPABILITY PATH, deliberately: pure compute, a host-brokered network
  // call, and a host-brokered model call. Clicking all three is the sandbox smoke test.
  const SUGGESTIONS = [
    'Roll 4d6 and drop the lowest',
    'Look up the owlbear',
    'Narrate our party entering a flooded crypt',
  ]
  ...
  // `tools.invoke` runs ONE tool with no LLM in the loop. Errors are rendered verbatim and not
  // prettified: when a tool is refused by the sandbox the message IS the diagnosis ("host X is
  // not one this plugin may reach", "this tool is not granted model access"), and rewording it
  // into "something went wrong" would throw away the only useful thing on the screen.
  async function invoke(btn) {
    const out = $('probeOut')
    const name = btn.dataset.tool
    const args = { [btn.dataset.arg]: ($(btn.dataset.src) || {}).value || '' }
    out.hidden = false
    out.textContent = `${name}…`
    btn.disabled = true
    const started = Date.now()
    try {
      const res = await client.invokeTool(name, args)
      const text = (res && (res.text ?? res.result ?? '')) || JSON.stringify(res, null, 2)
      out.textContent = `${name}  (${Date.now() - started}ms)${res && res.isError ? '  — ERROR' : ''}

${text}`
    } catch (e) {
      out.textContent = `${name}  — FAILED
...
${(e && e.message) || String(e)}`
  ...
  for (const b of document.querySelectorAll('.probe-go')) {
    b.addEventListener('click', () => void invoke(b))
  }

  for (const b of document.querySelectorAll('.nav-item')) {
      b.classList.toggle('active', b.dataset.view === next)
    }
    $('crumbRoot').textContent = next === 'settings' ? 'Settings' : next === 'tools' ? 'Tools' : 'Chat'
