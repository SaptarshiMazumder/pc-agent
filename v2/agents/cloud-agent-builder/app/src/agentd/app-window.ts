...
export async function buildAndOpen(
  ...
  // the browser blocks the pop-up. This is the whole cloud fix: opening after an await reads to
  // the browser as a script pop-up (blocked), opening synchronously reads as a click (allowed).
  // Desktop goes through the bridge and needs no pre-open; a null (pop-ups hard-blocked) falls
  // through to the helpful error in openAgentWindow.
