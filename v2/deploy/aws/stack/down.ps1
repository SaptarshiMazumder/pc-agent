...
if ($Full) {
  ...
  # Flat layout: the ALB is three resources now (lb + target groups + listeners).
  # Each -target must be a pre-quoted string (like $chdir); a bare -target=aws_lb.main gets
  # mangled by PowerShell's native-arg parsing and terraform sees a split target.
  $targets = "-target=aws_lb.main", "-target=aws_lb_target_group.svc", "-target=aws_lb_listener.svc"
