...
# terminal 2 — model gateway (custom auth ON)
$env:ACCOUNTS_URL = "http://127.0.0.1:4100"; $env:ACCOUNTS_INTERNAL_KEY = "devinternal"
.\.venv\Scripts\python.exe v2\deploy\gateway\run-local.py
...
exactly ONE row per model call (written by the gateway, not the daemon). Sign out →
Settings shows "Your own keys" and BYOK still works.

(Already verified in-repo: unit tests `v2/tests/test_platform_keys.py`, accounts hardening
...
Expect in the plan: ~4 new SG rules (ALB :4000/:443 + service :4000), a `gateway` target
...
for gateway/accounts. Success: outputs include `accounts_url`, `model_gateway_url`,
...
docker build -t <ecr>/gateway:latest  -f deploy/docker/Dockerfile.gateway  .
docker build -t <ecr>/accounts:latest -f deploy/docker/Dockerfile.accounts .
docker push <ecr>/gateway:latest; docker push <ecr>/accounts:latest

aws ecs update-service --cluster agentd-dev --service agentd-dev-gateway  --force-new-deployment
...
`model_gateway_url` from the terraform outputs.
