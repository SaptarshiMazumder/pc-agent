...
class FileBundleInstaller:
    def __init__(self, agents_dir: Path, plugins_dir: Path, state_dir: Path,
    ...
    async def install_files(self, package_path: Path, manifest: BundleManifest) -> list[str]:
        ...
        if missing:
            raise BundleError(f"'{manifest.id}': manifest declares vendored plugin(s) the zip "
    ...
    async def remove_files(self, installed: InstalledBundle, keep_plugin_ids: set[str],
        ...
        if agent_dir.is_dir():
            ...
            if (not purge_state and workspace.is_dir()
        ...
        for plugin_id in installed.plugin_ids:
            if plugin_id in keep_plugin_ids:
                log.info("uninstall %s: keeping plugin '%s' (another bundle uses it)",
    ...
    def _check_builtin_deps(self, manifest: BundleManifest) -> None:
        ...
        for dep in manifest.plugins:
            ...
            for root in (self._builtin_plugins_dir, self._plugins_dir):
                ...
                raise BundleError(f"'{manifest.id}' needs built-in plugin '{dep.id}', which this "
