...
def load_native_plugin(manifest: PluginManifest, config) -> list:
    """Import ``manifest.entry`` ("module:func"), call ``func(api, ctx)``, return its tools.
    The plugin's own dir is put on ``sys.path`` first so a drop-in folder is importable."""
    ...
    try:
        ...
        register(api, PluginContext(config=config))
        log.info("plugins: loaded native plugin '%s' (%d tool(s))", manifest.id, len(api.tools))
        return api.tools
    except Exception as e:  # noqa: BLE001 — a broken plugin must never break the rest
        log.warning("plugins: failed to load native plugin '%s': %s", manifest.id, e)
        return []
