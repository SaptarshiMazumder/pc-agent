...
come from env (AGENTD_MODEL_GATEWAY_URL), config (model_gateway.api_base), or the distribution
profile ([platform] model_gateway_url); first one wins in that order.

Enablement: env URL present, or config opted in (model_gateway.enabled), or the URL came from
...
def configure(config) -> None:
    """Read the gateway settings, at boot or at runtime (platform.connect re-invokes after
    ...
    env_url = os.environ.get("AGENTD_MODEL_GATEWAY_URL", "").strip()
    cfg_url = str(mg.get("api_base") or "").strip()
    dist_url = str(getattr(profile, "model_gateway_url", "") or "").strip()
    url, _source = next(
...
def apply(kwargs: dict) -> dict:
    ...
    from agentd.infrastructure import accounts  # local import — accounts imports nothing from llm
