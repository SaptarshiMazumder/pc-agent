...
class AddMcpTool(Tool):
    ...
    parameters = {
        ...
        "properties": {
            "name": {"type": "string", "description": "a short id for the server (its tool namespace)"},
    ...
    async def execute(self, tool_call_id, params, abort, on_update=None):
        ...
        if not (params.get("command") or params.get("url")):
            return ToolResult.text("add_mcp needs a 'command' (stdio) or 'url' (http)", is_error=True)
        try:
            result = await self._add({
                "name": name, "command": params.get("command"), "url": params.get("url"),
                "env": params.get("env"), "headers": params.get("headers"),
            })
        ...
        return ToolResult.text(
            ...
            + ", ".join(tools[:20]) + (" …" if len(tools) > 20 else ""),
            details=result)
