"""list_plugins — print the live PLUGIN -> TOOL catalog, generated from what tools declare in code.
  ...
  python -m agentd.main.list_plugins --json     # machine-readable JSON (for docs / tooling)
  python -m agentd.main.list_plugins --scaffold # ADD every plugin+tool to agentd.config.json's
...
in the human `description` fields from agentd.config.json's `plugins` block. So it can never drift
