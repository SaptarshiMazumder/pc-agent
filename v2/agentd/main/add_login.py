"""add_login — securely capture a site's login for an agent into the encrypted vault (one-time).
  ...
  python -m agentd.main.add_login --genkey            # print a vault master key for AGENTD_VAULT_KEY
  python -m agentd.main.add_login <agent> <site>      # prompt for url / username / password (no echo)
...
def main() -> None:
    ...
    if args and args[0] in ("--genkey", "-g"):
        ...
        print("# ^ set this as AGENTD_VAULT_KEY (env / .env) — keep it secret, never commit it",
              file=sys.stderr)
        return
    if len(args) < 2:
        print("usage: python -m agentd.main.add_login <agent> <site>   (or --genkey)", file=sys.stderr)
        sys.exit(2)

    agent, site = args[0].strip(), args[1].strip()
    cfg = load_config()                       # loads v2/.env -> AGENTD_VAULT_KEY into the env
    from agentd.infrastructure.credentials import build_credential_store
    store = build_credential_store(cfg)
    if store is None:
        print("AGENTD_VAULT_KEY is not set. Run `python -m agentd.main.add_login --genkey`, "
              "set it in your .env, and retry.", file=sys.stderr)
        sys.exit(1)
    ...
    store.put(agent, Credential(
        site=site, login_url=login_url, username=username, password=password,
        user_selector=user_sel, pass_selector=pass_sel, otp_selector=otp_sel))
