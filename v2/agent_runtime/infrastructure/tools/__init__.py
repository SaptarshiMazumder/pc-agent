...
``agent_runtime.infrastructure.plugins``. The core contributes NO tool implementations; ``build_tools`` is
gone (the catalog is assembled entirely by plugin discovery — see ``main/container.py``).

This package keeps only the tool FRAMEWORK, re-exported here so ``from agent_runtime.infrastructure.tools
import Tool`` keeps working:

  * ``Tool`` / ``ToolResult`` / ``OnUpdate`` / ``ToolArgError`` / ``validate_args`` — the contract,
    canonically defined in ``agent_runtime.application.interfaces.tool`` (clean-arch home: an interface
...
from agent_runtime.application.interfaces.tool import (  # noqa: F401
    OnUpdate,
    Tool,
    ToolArgError,
    ToolResult,
