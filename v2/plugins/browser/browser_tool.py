...
class BrowserTool(Tool):
    ...
    # provider = the browser ENGINE (single pick), self-described so a client shows a dropdown, not a
    # text box. See config.resolve_browser_engine (default "playwright"; "agent_browser" opts into the
    # external Vercel engine via its MCP server).
    provider_options = ["playwright", "agent_browser"]
    parameters = {
        "type": "object",
        "required": ["action"],
        "properties": {
            ...
            "target_id": {
                "type": "string",
                "description": "Tab handle: a label or tabId (e.g. 't2'). Omit = active tab.",
            },
            ...
            "mode": {
                "type": "string",
                "enum": ["efficient"],
                "description": "efficient snapshot preset.",
            },
            "interactive": {
                "type": "boolean",
                "description": "Snapshot: focus interactive elements.",
            },
            "compact": {
                "type": "boolean",
                "description": "Snapshot: drop unnamed structural nodes.",
            },
            ...
            "refs": {
                "type": "string",
                "enum": ["aria", "role"],
                "description": "Ref style (aria = durable, default).",
            },
            "urls": {"type": "boolean", "description": "Snapshot: include link /url targets."},
            "labels": {
                "type": "boolean",
                "description": "Snapshot/screenshot: include [box] coordinates.",
            },
            "selector": {
                "type": "string",
                "description": "Scope snapshot / wait to a CSS selector subtree.",
            },
            "frame": {
                "type": "string",
                "description": "CSS selector of an iframe to operate inside.",
            },
            # act
            "kind": {
                "type": "string",
            ...
            "modifiers": {
                "type": "array",
                "items": {"type": "string"},
                "description": "click: held keys (Control/Shift/Alt/Meta).",
            },
            "slowly": {
                "type": "boolean",
                "description": "type: per-character typing (React/contenteditable).",
            },
            "delay_ms": {
                "type": "integer",
                "minimum": 0,
                "description": "type/press: inter-key delay.",
            },
            "fields": {
                "type": "array",
                "items": {"type": "object"},
                "description": "fill: [{ref|selector, text|value}, ...] — fill many in one act.",
            },
            # wait
            ...
            "element": {
                "type": "string",
                "description": "screenshot: CSS selector of a single element to capture.",
            },
            # console
            "limit": {
                "type": "integer",
                "minimum": 1,
                "description": "console: max recent messages.",
            },
            # dialog
            "accept": {
                "type": "boolean",
                "description": "dialog: accept (true) or dismiss (false) the next dialog(s).",
            },
            "prompt_text": {
                "type": "string",
                "description": "dialog: text to enter for a prompt() dialog.",
            },
            # upload
            "paths": {
                "type": "array",
                "items": {"type": "string"},
                "description": "upload: local file paths.",
            },
            "input_ref": {
                "type": "string",
                "description": "upload: ref/selector of the file <input>.",
            },
            # tabs (back-compat)
    ...
    async def _act(self, params) -> ToolResult:
        ...
        if not ref:
            return ToolResult.text(
                f"act kind={kind} requires ref (or fields for fill)", is_error=True
            )
        ...
        try:
            if kind == "click":
                ...
                await loc.fill(
                    params.get("text") or "", **({"timeout": timeout} if timeout else {})
                )
            elif kind == "type":
                if params.get("slowly"):
                    await loc.press_sequentially(
                        params.get("text") or "", delay=params.get("delay_ms")
                    )
                else:
                ...
                await loc.press(
                    params.get("key") or "Enter",
                    **({"delay": params["delay_ms"]} if params.get("delay_ms") else {}),
                )
                await mgr.settle()
            elif kind == "select":
    ...
    async def _action_failure_hint(self, page, ref) -> str:
        ...
        try:
            ...
            if await loc.count() == 0:
                return (
                    f"ref '{ref}' is stale (no longer on the page) — take a fresh snapshot "
            ...
            if state == "covered":
                return (
                    f"it is covered by <{info.get('by')}> at its click point — dismiss that "
                    f"overlay/popup (or scroll it away), then re-snapshot and retry."
                )
            if state in ("hidden", "offscreen"):
                return (
                    "it is off-screen/not visible — scroll it into view (act scrollIntoView "
                    "or evaluate window.scrollBy), then re-snapshot and retry."
                )
            return (
                "it didn't become clickable in time — the page likely changed; take a fresh "
                "snapshot and retry."
            )
    ...
    async def _screenshot(self, params) -> ToolResult:
        ...
        if params.get("element") or params.get("ref"):
            loc = (
                mgr.resolve_ref(params["ref"], page)
                if params.get("ref")
                else page.locator(params["element"])
            )
    ...
    async def _tabs(self, action, params) -> ToolResult:
        ...
        if action == "open":
            ...
            snap = (
                await self._snapshot_text(params, handle)
                if params.get("url")
                else "Opened blank tab."
            )
