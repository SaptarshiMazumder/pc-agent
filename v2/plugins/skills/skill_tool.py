...
class SkillWorkshopTool(Tool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "name": {
                "type": "string",
                "description": "skill id, kebab-case (e.g. suumo-deal-watch)",
            },
    ...
    async def execute(self, tool_call_id, params, abort, on_update=None):
        ...
        if action == "list":
            names = (
                sorted(p.parent.name for p in target.glob("*/SKILL.md")) if target.exists() else []
            )
            return ToolResult.text(
                "Skills:\n" + "\n".join(f"- {n}" for n in names) if names else "(no skills yet)"
            )
        ...
        return ToolResult.text(
            f"skill '{name}' saved ({d / 'SKILL.md'}) — available next turn.",
            details={"name": name},
        )
