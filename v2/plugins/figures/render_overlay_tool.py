"""render_overlay: turn a high-level annotation spec into editable SVG (and optionally a PNG).
...
an image) lets the agent SEE and refine it, and is what compose_layers lays over artwork.
"""
...
class RenderOverlayTool(Tool):
    ...
    description = (
        ...
        "dash. Use edges[].points from route_graph as an arrow's `points`.\n"
    ...
    async def execute(self, tool_call_id, params, abort, on_update=None):
        try:
            ...
            return ToolResult.text(f"render_overlay failed: {e}", is_error=True)
