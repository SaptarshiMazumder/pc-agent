...
def _to_pixels(raw_anchors, W, H):
    ...
    for a in raw_anchors:
        ...
        out.append({"label": a.get("label"), "target": target or [0.0, 0.0], "at": at,
                    "side": side, "present": present})
    return out
...
class ReadLabelsTool(Tool):
    ...
    parameters = {
        ...
        "properties": {
            ...
            "base": {"type": "string", "description": "Optional path to the TEXTLESS base to overlay on (your generated textless art). Anchors come back in ITS space, pointer snapped to its ink. OMIT for a standalone labelled image and read_labels_from_image will strip the labels off `image` via Gemini to make a clean base (returned as `base_png`)."},
            "snap": {"type": "boolean", "description": "Snap each pointer target onto the base's nearest drawn pixel so dots land on the structure. Default true. (The label position `at` is never snapped — it stays in the margin.)"},
    ...
    def _run(self, params: dict) -> dict:
        ...
        if params.get("structures"):
            expected = ("Expected labels (read these; omit any not actually drawn):\n"
        ...
        if params.get("snap", True):
            ...
            for a in anchors:
                if a.get("present", True) and a["target"] != [0.0, 0.0]:
                    a["target"], moved = _snap_to_ink(grid, cols, rows, cell, a["target"], None, W, H)
                    snapped += int(moved)
        ...
        for a in anchors:
            ...
            el = {"kind": "annotation", "text": a.get("label") or "", "target": a["target"], "at": at}
            if a.get("side"):
                el["side"] = a["side"]
            elements.append(el)

        present = [a for a in anchors if a.get("present", True)]
        return {"width": Wc, "height": Hc, "anchors": anchors, "elements": elements,
                "base_png": str(base), "stripped": stripped, "found": len(present), "snapped": snapped}
    ...
    async def execute(self, tool_call_id, params, abort, on_update=None):
        ...
        made = (f" Made a clean textless base by stripping the labels -> {r['base_png']}."
                if r.get("stripped") else "")
        ...
        return ToolResult.text(
            ...
            f"Draw `elements` with render_editable_overlay over the base (`base_png`), then compose_figure_layers.", details=r)
