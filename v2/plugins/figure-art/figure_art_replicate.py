...
def _endpoint(family, mode, tier, control_type, override):
    ...
    return (
        table.get(tier)
        or table.get(DEFAULT_TIER.get(family, "default"))
        or next(iter(table.values()))
    )


def _build_input(
    family, mode, prompt, aspect_ratio, ref_file, strength, steps, lora_url, lora_scale
):
    inp = {"prompt": prompt}
    if family == "flux":
