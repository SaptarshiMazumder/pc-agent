...
productName: ${name}
# The product's version is the AGENT's, not the shell's. Without this override electron-builder
# takes it from clients/desktop/package.json, so every per-agent installer claimed the shell
# version (0.1.5) no matter what the agent declared — "Figure Creator Setup 0.1.5.exe" for an
# agent whose agent.toml said 1.1.0, and the same number on every other agent built that day.
# The version is also how the publisher tooling matches a delivered installer to the bundle it
# belongs to (agentd bundle publish -> _stage_installers), so a wrong one here means every
# correct publish prints a mismatch warning.
extraMetadata:
  version: ${version}
