...
def test_scoped_invoke_runs_private_tool(tmp_path):
    ...
    gw = Gateway(
        ...
        service=SimpleNamespace(
            find_tool=lambda n, a=None: _ExecPrivTool() if (n == "priv_exec" and a == "demo") else None
        ),
        registry=SimpleNamespace(get=lambda a: spec if a == "demo" else (_ for _ in ()).throw(KeyError(a))),
    )
