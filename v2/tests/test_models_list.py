...
def test_kind_catalogs_are_separated():
    cfg = SimpleNamespace(plugins={}, model="gemini/gemini-3.1-pro-preview",
                          model_catalog=None, model_fallbacks=[], cost_efficiency={})
    cats = gw._kind_catalogs(cfg)
    assert set(cats) >= {"models", "text", "vision", "image", "embedding"}
    img = {o["value"] for o in cats["image"]}
...
def test_models_list_reports_brain_and_tool_models_with_kind_and_key():
    ...
    assert "image" in out["catalogs"] and "text" in out["catalogs"]


def _models_payload(plugins, cost_efficiency=None):
    cfg = SimpleNamespace(plugins=plugins, model="gemini/gemini-3.1-pro-preview",
                          model_catalog=None, model_fallbacks=[],
...
def test_models_list_splits_the_brain_when_cost_efficiency_on():
    out = _models_payload({}, cost_efficiency={"enabled": True,
