...
def _sk(i):
    return Skill(name=f"skill{i}", description="a fairly wordy description here", path=f"/p/s{i}/SKILL.md")


def test_full_format_when_under_budget():
    lines = _advertise_skills([_sk(i) for i in range(5)], 150, 18000)
    assert len(lines) == 5 and "a fairly wordy description here" in lines[0]   # full, with descriptions
...
def test_skills_section_applies_budget():
    ...
    assert "more skills" in p and "a fairly wordy description here" not in p     # degraded under budget
