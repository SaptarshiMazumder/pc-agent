...
def test_cron_expression_next_fire_with_tz():
    ...
    task = _task(
        kind="cron", cron_expr="55 19 * * 6", tz="Asia/Tokyo", every_seconds=None, next_due=wed
    )
...
async def test_cron_tool_add_every_lists_under_calling_agent(tmp_path):
    ...
    res = await tool.execute(
        "c", {"action": "add", "every": "15m", "payload": "check inbox"}, asyncio.Event()
    )
...
async def test_cron_tool_daily_anchors_to_local_time(tmp_path):
    ...
    await tool.execute(
        "c", {"action": "add", "daily": "19:30", "payload": "spending report"}, asyncio.Event()
    )
    t = store.list("main")[0]
...
async def test_cron_tool_add_cron_expression(tmp_path):
    ...
    r = await tool.execute(
        "c",
        {"action": "add", "cron": "55 19 * * 6", "tz": "Asia/Tokyo", "payload": "weekly report"},
        asyncio.Event(),
    )
    ...
    bad = await tool.execute(
        "c", {"action": "add", "cron": "not a cron", "payload": "x"}, asyncio.Event()
    )
...
async def test_cron_tool_get_update_run(tmp_path):
    ...
    upd = await tool.execute(
        "c", {"action": "update", "id": tid, "daily": "08:00", "payload": "new"}, asyncio.Event()
    )
...
async def test_cron_tool_deliver_message_outbox(tmp_path):
    ...
    await tool.execute(
        "c",
        {"action": "add", "in": "1h", "payload": "Happy birthday!", "deliver": "message"},
        asyncio.Event(),
    )
    t = store.list("main")[0]
...
def test_store_run_outcome(tmp_path):
    store = SqliteTaskStore(tmp_path / "a.sqlite")
    store.finish_run(
        store.record_run("t1", "main"), "blocked", outcome="blocked", detail="needs Drive auth"
    )
    r = store.recent_runs(task_id="t1")[0]
...
def test_gateway_cron_runs_full_history(tmp_path):
    ...
    assert (
        durs["running"] is None and durs["ok"] is not None and durs["error"] is not None
    )  # finished have a duration
    assert all("outcome" in r and "detail" in r for r in out["runs"])  # outcome surfaced

    # a blocked outcome shows through cron.runs
    store.finish_run(
        store.record_run("t3", "alpha"), "blocked", outcome="blocked", detail="needs Drive auth"
    )
