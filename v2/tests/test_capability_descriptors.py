...
def _registry(tmp_path, agents: dict):
    ...
    cfg = SimpleNamespace(state_dir=tmp_path / "state", agents_dir=adir, workspace=tmp_path,
                          agent_name="", skills_dir=adir / "main" / "skills")
...
def test_agent_description_fallback_chain(tmp_path):
    reg = _registry(tmp_path, {
        ...
        "viaident": {"agent.toml": 'name = "Y"\n', "IDENTITY.md": "# Y\n\nYou are Y, a specialist.\n"},
        "viabundle": {"agent.toml": 'name = "Z"\n', "bundle.toml": '[bundle]\ndescription = "from bundle"\n'},
    })
    assert reg.get("explicit").description == "from toml"                 # explicit field wins
    assert reg.get("viaident").description == "You are Y, a specialist."  # -> IDENTITY.md first prose line
...
def test_descriptor_producers_shape():
    specs = [SimpleNamespace(id="a1", name="Agent One", description="does one thing",
                             tagline="t", color="", model=None, dir="/x")]
    (agent,) = cap.agent_descriptors(specs)
    assert (agent.kind, agent.id, agent.description) == ("agent", "a1", "does one thing")

    catalog = {"figures": {"name": "Figures", "description": "figure core",
...
def test_roster_requires_a_delegation_tool(tmp_path):
    reg = _registry(tmp_path, {"figure-creator": {"agent.toml": 'name = "Figure Creator"\ndescription = "figures"\n'}})
    svc = _service(reg)
    main = reg.get("main")
    assert svc._agents_roster_section(main, [SimpleNamespace(name="read")]) == ""   # no delegation tool
