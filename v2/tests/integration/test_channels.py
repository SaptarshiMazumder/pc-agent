...
def test_build_channel_line_from_config_creds():
    ...
    assert (
        ch.agent_id == "restaurant" and ch.policy == "open" and ch.webhook_path == "/line/webhook"
    )
    ...
    ch = build_channel(
        {
            ...
            "allow_from": ["Uo"],
            "env_suffix": "owner",
            "webhook_path": "/line-owner/webhook",
        },
        None,
    )
    assert isinstance(ch, LineChannel)
...
def test_parse_search_handles_json_shapes_and_garbage():
    ...
    assert (
        parse_search('{"messages": [{"messageId": "m2", "sender": "b@x", "body": "yo"}]}')[0][
            "from"
        ]
        == "b@x"
    )
...
async def test_email_channel_send_and_poll_via_fake_invoke():
    ...
    assert (
        "google__send",
        {"to": "cust@x.com", "subject": "Re: your message", "body": "here you go"},
    ) in calls
