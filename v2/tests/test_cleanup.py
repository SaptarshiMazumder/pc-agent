...
from agentd.infrastructure.workspace.cleanup import (
    SCRATCH_DIRNAME, cleanup, plan_cleanup, sweep_scratch,
)
...
def test_sweep_scratch_deletes_old_keeps_new(tmp_path):
    ws = _ws(tmp_path)
    old = ws / "tmp" / "old.txt"; old.write_text("x", encoding="utf-8")
    new = ws / "tmp" / "new.txt"; new.write_text("y", encoding="utf-8")
...
def test_sweep_scratch_noop_when_disabled_or_absent(tmp_path):
    ws = _ws(tmp_path)
    f = ws / "tmp" / "a.txt"; f.write_text("x", encoding="utf-8")
