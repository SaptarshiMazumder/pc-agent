...
def test_bank_save_search_get(tmp_path):
    ...
    mid = bank.save(MemoryItem(id="", agent_id="scout", source="note",
...
def test_bank_persists_across_reopen(tmp_path):
    p = tmp_path / "m.sqlite"
    SqliteMemoryBank(p).save(MemoryItem(id="x", agent_id="A", source="note", text="durable across runs"))
...
def test_consolidate_dedups_per_agent(tmp_path):
    ...
    bank.save(MemoryItem(id="4", agent_id="B", source="note", text="prefers dark mode"))      # other agent
    assert consolidate(bank, "A") == 1                           # one dup removed
    assert {" ".join(i.text.split()).lower() for i in bank.recent("A")} == {
        "prefers dark mode", "lives in tokyo"}
...
async def test_memory_tools_roundtrip_scoped_to_agent(tmp_path):
    ...
    r = await RememberTool(bank).execute("c", {"text": "user prefers concise emails"}, asyncio.Event())
