...
def test_add_rejects_missing_command_and_dupes(monkeypatch):
    ...
    assert "already exists" in asyncio.run(gw._mcp_add({"name": "notion", "command": ["y"]}))["error"]
...
def test_list_shows_servers_with_connected_state():
    ...
    gw = _gw(servers=[McpServerConfig(name="notion", command=["x"]),
                      McpServerConfig(name="slack", command=["y"])], service=svc)
    out = gw._mcp_list()
...
def test_persist_writes_servers_and_preserves_other_keys(tmp_path, monkeypatch):
    ...
    cfgfile.write_text(json.dumps({"channels": [{"type": "line"}], "webhook_port": 8788}), encoding="utf-8")
    ...
    assert data["mcp_servers"] == [{"name": "notion", "transport": "stdio",
                                    "command": ["npx", "pkg"], "env": {"T": "1"}}]


def test_server_dict_omits_empty():
    assert _server_dict(McpServerConfig(name="n", url="http://x")) == \
        {"name": "n", "transport": "stdio", "url": "http://x"}
