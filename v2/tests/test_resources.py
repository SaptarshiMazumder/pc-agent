...
def test_enrich_runs_in_background_not_inline(tmp_path):
    ...
    assert store.get("a", "chart.png").description == "a bar chart of monthly costs"  # enriched later
...
def test_rich_fn_skips_non_images_without_api_call(monkeypatch):
    ...
    fn = build_rich_fn(SimpleNamespace(resource_vision_enabled=True,
...
def test_rich_fn_skips_text_when_summarize_off(monkeypatch):
    ...
    from agentd.infrastructure.resources.vision import build_rich_fn
    monkeypatch.setenv("GEMINI_API_KEY", "test-key")
    fn = build_rich_fn(SimpleNamespace(resource_vision_enabled=True, resource_summarize_enabled=False,
                                       resource_vision_model="m", resource_vision_timeout_seconds=5))
...
def test_resource_tool_crud_via_run_context(tmp_path):
    ...
    try:
        out = asyncio.run(tool.execute("c", {"action": "create", "path": "a.py",
                                             "content": "# hi\n"}, asyncio.Event()))
