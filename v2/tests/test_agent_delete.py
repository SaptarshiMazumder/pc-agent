...
def test_memory_purge_agent_only_targets_that_agent(tmp_path):
    ...
    bank.save(MemoryItem(id="", agent_id="scout", source="note", text="alpha", created_at=time.time()))
    bank.save(MemoryItem(id="", agent_id="scout", source="note", text="beta", created_at=time.time()))
    bank.save(MemoryItem(id="", agent_id="main", source="note", text="keep me", created_at=time.time()))
...
def test_task_store_purge_agent_clears_all_ledgers(tmp_path):
    ...
    store.add(ScheduledTask(id="t1", agent_id="scout", session_key="agent:scout:x:y",
                            kind="every", payload="ping", next_due=time.time(), every_seconds=60))
    store.add(ScheduledTask(id="t2", agent_id="main", session_key="main",
                            kind="every", payload="ping", next_due=time.time(), every_seconds=60))
...
def _registry(tmp_path):
    ...
    cfg = SimpleNamespace(
        agents_dir=agents_dir, state_dir=tmp_path / ".agentd",
        workspace=tmp_path / "ws", agent_name="main",
    )
...
class _Bank:
    def __init__(self): self.purged = None
    def purge_agent(self, agent_id): self.purged = agent_id; return 3
...
class _Reg:
    def __init__(self): self.removed = None
    def list_ids(self): return ["main", "scout"]
...
def _gw(reg, store, bank):
    return Gateway(config=SimpleNamespace(agent_id="main", agent_name="main"),
                   service=None, registry=reg, task_store=store, memory_bank=bank)
