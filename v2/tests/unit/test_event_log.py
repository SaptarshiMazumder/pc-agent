...
def test_emit_writes_one_file_per_run_named_by_agent(tmp_path):
    ...
    assert f.is_file()
    recs = _records(f)
    assert [r["event"]["type"] for r in recs] == [
        "agent_start",
...
def test_build_event_log_gated_by_flag(tmp_path):
    ...
    el = build_event_log(
        SimpleNamespace(event_log_enabled=True, state_dir=tmp_path, event_log_max_runs=50)
    )
...
async def test_gateway_feeds_the_event_log(tmp_path):
    ...
    class FakeService:
        async def handle_message(
            self, sk, message, on_event, abort, mode=None, agent_id=None, attachments=None
        ):
            await on_event(AgentEvent("agent_start", {}))
            await on_event(AgentEvent("agent_end", {"stopReason": "stop"}))

    gw = Gateway(
        config=SimpleNamespace(state_dir=tmp_path), service=FakeService(), event_log=FakeLog()
    )
