// forked from design-sync lib/source-kit.mjs — synth entry re-exports default exports under
// their declared names (this repo's components are `export default function Name`, which
// `export * from` never re-exports, leaving window.<GLOBAL> with no components at all).
//
  ...
  if (!entry) {
    ...
    const comps = srcFiles.filter(
      (p) =>
        SRC_IMPL_RX.test(p) &&
        !NON_IMPL_RX.test(p) &&
        // THE FORK (2/2): this package is an APP — src/main.tsx is its bootstrap and calls
        // `createRoot(document.getElementById('root')).render(...)` at module scope. Bundled
        // into the entry, that throws on any page without a #root and takes the whole IIFE
        // down with it: window.<GLOBAL> is then never assigned and every component reads as
        // missing.
        !/[\\/]main\.[tj]sx$/.test(p),
    );
    entry = join(OUT, '.pkg-entry.mjs');
    // THE FORK (1/2): `export *` skips default exports, and this repo's components are default
    // exports — recover the declared name textually and re-export by it.
    const entryLines = [];
    // THE FORK (3/3): the excluded bootstrap was also the file importing the app's three CSS
    // layers (tokens → theme → styles, order load-bearing: theme overrides tokens). Import them
    // from the entry instead, so the bundle CSS carries the tokens and the @fontsource faces
    // exactly as the app itself does.
    for (const css of ['tokens.css', 'theme.css', 'styles.css']) {
      const cssPath = join(srcRoot, css);
      if (existsSync(cssPath)) entryLines.push(`import ${JSON.stringify(slash(cssPath))};`);
    }
